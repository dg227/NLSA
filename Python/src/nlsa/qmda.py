...
def make_da(
    forecast: Callable[[X], X], analysis: Callable[[Y, X], X]
) -> Callable[[Y, X], X]:
...
def make_pure_state_prediction(
    t: ImplementsPureState[Y, A], a: A
) -> Callable[[Y], K]:
...
@runtime_checkable
class ImplementsComposeAndPureState(
    ImplementsCompose[Y, A_contra, Y],
...
def pure_state_prediction(
    t: ImplementsComposeAndPureState[Y, A], a: A, u: A, xi: Y
) -> K:
