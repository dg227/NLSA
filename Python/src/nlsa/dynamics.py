...
def orbit[X](x0: X, f: Callable[[X], X]) -> Generator[X]:
...
def cocycle_orbit[X, Y](
    y0: Y, xs: Iterable[X], f: Callable[[X, Y], Y]
) -> Generator[Y]:
...
def from_autonomous[X, TX, **P](
    v: Callable[[X], TX],
) -> Callable[Concatenate[object, X, P], TX]:
    """Make time-dependent vector field from autonomous vector field."""

    def f(_: object, x: X, *args: P.args, **kwargs: P.kwargs) -> TX:
        y = v(x)
        return y
