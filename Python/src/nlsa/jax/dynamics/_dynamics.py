...
def make_fin_orbit(
    f: Callable[[X], X], num_steps: int = 1
) -> Callable[[X], Xs]:
...
def vgrad[X: Array](g: Callable[[X], R]) -> Callable[[X, TX], R]:
...
def flow(
    v: Callable[[X], TX], t: Real, /, dt0: Optional[Real] = None
) -> Callable[[X], X]:
    """Compute time-t flow generated by vector field."""

    def f(_: Real, x: X, args: Array) -> X:
        y = v(x)
        return y
...
def make_posfreq_to_l2(
    k_max: int, dtype: Optional[DTypeLike] = None
) -> Callable[[Vpos], V]:
...
def make_stepanoff_vector_field(
    freq: float, dtype: Optional[DTypeLike] = None
) -> Callable[[T2], R2]:
    """Make vector field of the Stepanoff flow on the 2-torus."""

    def vec(x: T2) -> R2:
        y = jnp.empty(2, dtype=dtype)
        y = y.at[1].set(freq * (1 - jnp.cos(x[0] - x[1])))
        y = y.at[0].set(y[1] + (1 - freq) * (1 - jnp.cos(x[1])))
        return y
...
def make_rotation_vector_field[D: DTypeLike](
    freqs: float | Sequence[float] | ArrayLike, dtype: Optional[D] = None
) -> Callable[[Tn], Rn]:
...
def make_rotation_map[D: DTypeLike](
    angles: float | Sequence[float] | ArrayLike,
    mod: float = 2 * jnp.pi,
...
def make_s1_rotation_generator_fourier(
    freq: float,
    max_wavenum: int,
    hermitian: bool = False,
    dtype: Optional[DTypeLike] = None,
) -> Callable[[V], V]:
    """Make rotation generator in the Fourier basis of the circle."""
    if hermitian:
        const = jnp.array(1, dtype=dtype)
    else:
        const = jnp.array(1j, dtype=dtype)
    freq_ks = (
        const * freq * jnp.arange(-max_wavenum, max_wavenum + 1, dtype=dtype)
    )
...
def make_t2_rotation_generator_fourier(
    ...
    n1 = 2 * k_max[0] + 1
    n2 = 2 * k_max[1] + 1
    k1s = jnp.arange(-k_max[0], k_max[0] + 1, dtype=dtype)
    k2s = jnp.arange(-k_max[1], k_max[1] + 1, dtype=dtype)
    if hermitian:
        const = jnp.array(1, dtype=dtype)
    else:
        const = jnp.array(1j, dtype=dtype)
    ...
    def gen(u: V) -> V:
        ...
        b = (
            k1s[:, jnp.newaxis] * freqs[0] + k2s[jnp.newaxis, :] * freqs[1]
        ) * a
        return const * b.ravel()

    return gen


def make_stepanoff_generator_fourier(
    freq: float,
    max_wavenums: int | tuple[int, int],
    hermitian: bool = False,
    dtype: Optional[DTypeLike] = None,
    ...
    n1 = 2 * k_max[0] + 1
    n2 = 2 * k_max[1] + 1
    k1 = jnp.arange(-k_max[0], k_max[0] + 1, dtype=dtype)
    k2 = jnp.arange(-k_max[1], k_max[1] + 1, dtype=dtype)
    if hermitian:
        const = jnp.array(1, dtype=dtype)
    ...
    def gen(u: V) -> V:
        ...
        b = b.at[1:, :-1, 1].set(
            -const * 0.5 * freq * k1[:-1, jnp.newaxis] * a[:-1, 1:]
        )
        b = b.at[:-1, 1:, 2].set(
            -const * 0.5 * freq * k1[1:, jnp.newaxis] * a[1:, :-1]
        )
        b = b.at[:, 1:, 3].set(
            -const * 0.5 * (1 - freq) * k1[:, jnp.newaxis] * a[:, :-1]
        )
        b = b.at[:, :-1, 4].set(
            -const * 0.5 * (1 - freq) * k1[:, jnp.newaxis] * a[:, 1:]
        )
        ...
        b = b.at[:, :, 5].set(const * freq * k2 * a)
        b = b.at[1:, :-1, 6].set(-const * 0.5 * freq * k2[1:] * a[:-1, 1:])
        b = b.at[:-1, 1:, 7].set(-const * 0.5 * freq * k2[:-1] * a[1:, :-1])
...
def stepanoff_generator_matrix(
    alpha: float,
    k: int,
    hermitian: bool = False,
    dtype: Optional[DTypeLike] = None,
