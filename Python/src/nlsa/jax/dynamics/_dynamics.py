...
def flow(
    v: Callable[[X], TX], t: Real, /, dt0: Real | None = None
) -> Callable[[X], X]:
...
def make_posfreq_to_l2(
    k_max: int, dtype: DTypeLike | None = None
) -> Callable[[Vpos], V]:
...
def make_stepanoff_vector_field(
    freq: float, dtype: DTypeLike | None = None
) -> Callable[[T2], R2]:
...
def make_rotation_vector_field[D: DTypeLike](
    freqs: float | Sequence[float] | ArrayLike, dtype: D | None = None
) -> Callable[[Tn], Rn]:
