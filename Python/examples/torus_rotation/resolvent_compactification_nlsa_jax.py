# pyright: basic
"""Koopman spectral analysis of torus rotation: Resolvent compactification."""
...
kernel_pars = KernelPars(
    normalization="bistochastic", eigensolver="eigsh", num_eigs=256
)
...
@partial(plotem, io=io, mode=PLOT_MODE, fname="koopman_eigen")
def make_koopman_evecs_plotter[N: int, Ntst: int, D: DTypeLike](
    ...
    def plot_eigs(j: int):
        ...
        ax.set_title(f"Frequency $\\omega_{{{j}}} = {evl: .3f}$")
        ax.grid()
...
def main():
    """Resolvent-compactified Koopman spectral analysis of torus rotation."""
    print(
        f"-zT = {
