...
from nlsa.jax.utils import make_batched2
...
class DataPars[N: int]():
    ...
    def __str__(self) -> str:
        """Create string representing data parameters."""
        x0str = f"x0_{self.x0[0]:.2f}_{self.x0[1]:.2f}_{self.x0[2]:.2f}"
        return '_'.join((self.covariate,
                         self.response,
                         f"dt{self.dt:.2f}",
                         f"n{self.num_samples}",
                         f"nspinup{self.num_spinup}",
                         f"nfd{self.num_fd}",
                         f"ndl{self.num_half_delays}",
                         f"nquad{self.num_quad}",
                         f"npred{self.num_pred_steps}",
                         x0str))
...
class PredPars:
    ...
    def __str__(self) -> str:
        ...
        return '_'.join(("pred", "eigs" + eigs_str))
...
class TrainPars[N: int]:
    ...
    def __str__(self) -> str:
        """Create string representing training parameters."""
...
kernel_pars = KernelPars(normalization="bistochastic",
                         eigensolver="eigsh",
                         num_eigs=150)
koopman_pars = KoopmanParsQz(fd_order=4,
                             dt=train_data_pars.dt,
                             tau=0.5,
                             res_z=0.5,
                             laplace_method="log",
                             which_eigs_galerkin=1000,
                             num_eigs=128,
                             sort_by="energy",
                             quad_batch_size=1024,
                             gram_batch_size=None)
pred_pars = PredPars(which_eigs=300)
...
def to_koopman_eigen(dict_in: dict[str, ArrayLike],
                     dtype: Optional[DTypeLike] = None) -> KoopmanEigen:
...
def generate_data[N: int](pars: DataPars[N], dtype: DTypeLike) -> Data:
    ...
    num_total_samples = pars.num_samples + num_delays + pars.num_pred_steps \
        + pars.num_spinup + pars.num_fd + pars.num_quad
    with jax.default_device(device_cpu):
        if FP == "F32":
            jax.config.update("jax_enable_x64", True)
        ts_tot = jnp.arange(num_total_samples) * pars.dt
        solution = diffrax.diffeqsolve(
            terms=diffrax.ODETerm(jit(dyn.from_autonomous(v))),
            solver=diffrax.Dopri5(),
            t0=0, t1=ts_tot[-1], dt0=pars.dt,
            y0=jnp.array(pars.x0, dtype=dtype),
            saveat=diffrax.SaveAt(ts=ts_tot[pars.num_spinup:]),
...
@timeit
@partial(npyit, io=io, mode=QZ_MATRIX_MODE, fname="qz_mat", cls=Array,
         callback=partial(jnp.array, dtype=r_dtype))
def compute_qz_matrix[N: int, D: DTypeLike](
        pars: TrainPars[N], l2y: L2VectorAlgebra[tuple[N], D, Yd, R],
        train_data: Data,
        basis: alg.ImplementsDimensionedL2FnFrame[Yd, R, V, Rs, Array | int]) \
            -> M:
    ...
    i1 = i0 + pars.data.num_samples + pars.data.num_delays + pars.data.num_quad
    ...
    match pars.data.num_half_delays, pars.koopman.quad_batch_size:
        ...
        case 0, _:
            ...
            eval_at_yvs = vec.batch_eval_at(
                ...
                batch_size=pars.koopman.quad_batch_size)
        case _, None:
            eval_at_ys = dl.delay_eval_at(train_data['covariates'][i0:i1],
                                          num_delays=pars.data.num_delays)
            eval_at_yvs \
                = dl.delay_eval_at((train_data['covariates'][i0:i1], vs),
                                   num_delays=pars.data.num_delays)
        case _, _:
            ...
            eval_at_yvs = dl.batch_delay_eval_at(
                ...
                batch_size=pars.koopman.quad_batch_size,
                num_delays=pars.data.num_delays)
    lapl_vgrad_phi = compose(lapl_op,
                             compose(eval_at_yvs,
                                     compose(dyn.vgrad, basis.fn)))
    lapl_phi = compose(lapl_op,
                       compose(eval_at_ys, basis.dual_fn))
    ...
    if pars.koopman.gram_batch_size is not None:
        compute_qz_op_batched \
            = make_batched2(compute_qz_op,
                            max_batch_sizes=(pars.koopman.gram_batch_size,
...
compute_koopman_eigen = timeit(h5it(koop.compute_eigen_qz, io=io,
                                    mode=KOOPMAN_EIGEN_MODE,
                                    fname="koopman_eigen_qz", cls=KoopmanEigen,
                                    callback=to_koopman_eigen))


def make_timeseries_prediction_function[N: int, Ntst: int](
        pars: Pars[N, Ntst], train_data: Data,
        koopman_basis: KoopmanEigenbasis[Yd, C, V, Cs, Array | int]) \
            -> F[Yd, Cs]:
    """Make vector-valued prediction function for time series prediction."""
    i0 = pars.train.data.delay_embedding_end
    i1 = i0 + pars.train.data.num_samples
    f_coeffs = koopman_basis.anal(train_data['responses'][i0:i1])
    ts = pars.test.data.dt * jnp.arange(pars.test.data.num_pred_steps + 1)
...
def compute_skill_scores[Ntst: int](pars: DataPars[Ntst], test_data: Data,
    ...
    nrmses = jit(vmap(normalized_rmse, in_axes=1))(fxs_true, fys_pred)
    accs = jit(vmap(anomaly_correlation, in_axes=1))(fxs_true, fys_pred)
...
def plot_bandwidth_func[N: int, Ntst: int, D: DTypeLike](
    ...
    if test_pars is not None and l2y_tst is not None and test_data is not None:
        ...
        sc_tst = ax_tst.scatter(
            ...
            c=bw_vals_tst[i0_tst:i1_tst:plt_step_tst], s=1, vmin=vmin,
...
plot_qz_matrix = plotit(koop.plot_operator_matrix, io=io, mode=PLOT_MODE,
                        fname="qz_mat")
plot_generator_spectrum = plotit(koop.plot_generator_spectrum, io=io,
                                 mode=PLOT_MODE, fname="gen_spec")


@partial(plotem, io=io, mode=PLOT_MODE, fname="zeta", block=False)
def make_koopman_evecs_plotter[N: int, Ntst: int, D: DTypeLike](
    ...
    if plt.fignum_exists(i_fig):
        plt.close(i_fig)
    if test_pars is not None:
        fig = plt.figure(num=i_fig, figsize=tuple(1.5 * mpf.figaspect(0.6)))
        axs = (fig.add_subplot(2, 4, 1, projection="3d"),
               fig.add_subplot(2, 4, 2, projection="3d"),
               fig.add_subplot(2, 4, 3),
               fig.add_subplot(2, 4, 4))
        axs_tst = (fig.add_subplot(2, 4, 5, projection="3d"),
    ...
    def plot_eig(j: int):
        ...
        sc = ax.scatter(train_data['states'][i0:i1:plt_step, 0],
                        train_data['states'][i0:i1:plt_step, 1],
                        train_data['states'][i0:i1:plt_step, 2],
                        c=evec.real[:num_plt:plt_step], s=1, vmin=-amax,
                        vmax=amax,
                        cmap="seismic")
        ax.set_xlabel("$x^1$")
        ax.set_ylabel("$x^2$")
        ...
        ax = axs[1]
        assert isinstance(ax, Axes3D)
        ax.scatter(train_data['states'][i0:i1:plt_step, 0],
                   train_data['states'][i0:i1:plt_step, 1],
                   train_data['states'][i0:i1:plt_step, 2],
                   c=evec.imag[:num_plt:plt_step], s=1, vmin=-amax, vmax=amax,
                   cmap="seismic")
        ax.set_xlabel("$x^1$")
        ax.set_ylabel("$x^2$")
        ax.set_zlabel("$x^3$")
        ...
        ax = axs[2]
        ax.plot(evec.real[::plt_step], evec.imag[::plt_step], "-")
        ...
        ax = axs[3]
        ax.plot(ts[::plt_step], evec.real[:num_plt:plt_step],
                "-", label=f"$\\mathrm{{Re}}\\zeta_{{{j}}}$")
        ax.plot(ts[::plt_step], evec.imag[:num_plt:plt_step],
        ...
        if test_pars is not None and l2y_tst is not None \
            ...
            sc_tst = ax.scatter(
                ...
                test_data['states'][i0_tst:i1_tst:plt_step_tst, 1],
                test_data['states'][i0_tst:i1_tst:plt_step_tst, 2],
                c=evec_tst.real[:num_plt_tst:plt_step_tst], s=1, vmin=-amax,
                vmax=amax, cmap="seismic")
            ax.set_xlabel("$x^1$")
            ax.set_ylabel("$x^2$")
            ax.set_zlabel("$x^3$")
            ...
            ax.scatter(test_data['states'][i0_tst:i1_tst:plt_step_tst, 0],
                       test_data['states'][i0_tst:i1_tst:plt_step_tst, 1],
                       test_data['states'][i0_tst:i1_tst:plt_step_tst, 2],
                       c=evec_tst.imag[:num_plt_tst:plt_step_tst], s=1,
            ...
            ax = axs_tst[2]
            ax.plot(evec_tst.real[:num_plt_tst:plt_step_tst],
...
def make_pred_plotter[Ntst: int](test_pars: DataPars[Ntst],
        test_data: Data, preds: Vtsts, num_plt_tst: Optional[int] = None,
        plt_step_tst: int = 1, i_fig: int = 1) -> tuple[Figure, F[int, None]]:
...
def plot_forecast_skill_scores(scores: SkillScores, dt: float = 1,
                               i_fig: int = 1) -> Figure:
    ...
    ts = jnp.arange(scores['nrmses'].size) * dt
...
def main():
    ...
    kernel_basis = knl.make_eigenbasis(
        pars.train.kernel, l2y, kernel, kernel_eigen,
    ...
    koopman_eigen = compute_koopman_eigen(pars.train.koopman, qz_mat,
                                          kernel_basis)
    print(tabulate(
        jnp.vstack((koopman_eigen['evals'][:NUM_TABULATE].real,
                    koopman_eigen['engys'][:NUM_TABULATE],
                    koopman_eigen['efreqs'][:NUM_TABULATE],
                    koopman_eigen['eperiods'][:NUM_TABULATE])).T,
        headers=["Growth rate", "Dirichlet energies",
                 "Eigenfreqs.","Eigenperiods"],
        floatfmt=".4f", showindex=True))
    ...
    c_lk = VectorAlgebra(shape=(pars.train.koopman.dim_galerkin,),
                         dtype=c_dtype)
    koopman_basis = koop.make_eigenbasis_antisym(
        c_lk, kernel_basis, koopman_eigen,
    ...
    if PLOT_MODE is not None and KOOPMAN_EIGS_PLT is not None:
        _, plot_koopman_eig = make_koopman_evecs_plotter(
            pars.train.data, train_data, koopman_basis, pars.test.data,
            l2y_tst, test_data)
    ...
    io /= str(pars.test.data)
    predict = jit(make_timeseries_prediction_function(pars, train_data,
                                                      koopman_basis))
    ...
    print(skill_scores['nrmses'])
    print("Anomaly correlation coefficient:")
    print(skill_scores['accs'])

    # Plot forecast skill scores
    if PLOT_MODE is not None:
        plot_forecast_skill_scores(skill_scores, dt=pars.test.data.dt)
