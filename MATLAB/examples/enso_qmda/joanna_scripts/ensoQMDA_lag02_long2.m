...
ifPlotPrb    = false; % running probability forecast
...
[ model, In, modelObs, InObs, OutObs ] = ensoQMDAlong_nlsaModel_lag02( experiment );
...
if ifPlotExpStd
    ...
    for iY = 1 : nYPlt
        ...
        for iT = 1 : nTPlt 
            ...
            tPlt      = tNumVer( ( 1 : nTO ) + idxTPlt( iT ) - 1, : );
            tPlt      = tPlt( : )';
...
if ifPlotPrb
    ...
    for iY = 1 : nYPlt
        ...
        for iT = 1 : nTPlt 
            ...
            % Plot predicted mean 
            plot( tPlt, yExpPlt, 'k-', 'lineWidth', 2 )
...
if ifPlotEnt
    ...
    for iY = 1 : nYPlt
        for iT = 1 : nTPlt
            ...
            dKLPlt = squeeze( ...
                 dKL( idxYPlt( iY ), ( 1 : nTO ) + idxTPlt( iT ) - 1, : ) ); 
            dKLPlt = dKLPlt( : );

            eKLPlt = squeeze( ...
                 eKL( idxYPlt( iY ), ( 1 : nTO ) + idxTPlt( iT ) - 1, : ) ); 
            eKLPlt = eKLPlt( : );
...
if ifMovieProb
    ...
    for iT = 1 : nTPlt
        tPlt{ iT } = tNumVer( ( 1 : nTO ) + idxTPlt( iT ) - 1, : );
        tPlt{ iT } = tPlt{ iT }( : )';
    ...
    for iY = 1 : nYPlt
       for iT = 1 : nTPlt 
            pPlt{ iT, iY } = real( squeeze( ...
               p( :, idxYPlt( iY ), ( 1 : nTO ) + idxTPlt( iT ) - 1, : ) ) ); 
