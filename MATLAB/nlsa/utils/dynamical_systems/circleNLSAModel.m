function [ model, In, Out ] = circleNLSAModel( In, Out )
% CIRCLENLSAModel Build NLSA model for variable-speed flow on the circle
%
%  [ model, In ] = circleNLSAModel( In ) builds an NLSA model based on the
%  parameters specified in the structure In. 

%  [ model, In, Out ] = circleNLSAModel( In, Out ) builds an NLSA model
...
%
%  circleNLSAModel uses the parameter values in In and Out to create arrays of 
...
%
%  circleNLSAModel is meant to be called by higher-level functions tailored to 
...
for iR = In.nR : -1 : 1
    ...
    % Realization tag                                
    tagR = [ 'f'       num2str( In.Res( iR ).f, '%1.2f' ) ...
             '_a'      num2str( In.Res( iR ).a, '%1.2f' ) ...
             '_dt'     num2str( In.Res( iR ).dt ) ...
             '_nS'     int2str( In.Res( iR ).nS ) ...
             '_nSSpin' int2str( In.Res( iR ).nSSpin ) ...
             '_r1'     num2str( In.Res( iR ).r1, '%1.2f' ) ...
             '_r2'     num2str( In.Res( iR ).r2, '%1.2f' ) ...
    ...
    for iC = In.nC : -1 : 1
        ...
        % Input file
        if nD == 2 && all( In.Src( iC ).idxX == [ 1 2 ] )
            fileName = 'dataX.mat';
        else
    ...
    for iC = In.nCT : -1 : 1
        ...
        % Input file
        if nD == 2 && all( In.Trg( iC ).idxX == [ 1 2 ] )
            fileName = 'dataX.mat';
        else
            fileName = [ 'dataX_' tagC '.mat' ];
        end
...
if ifOse
    for iR = Out.nR : -1 : 1
        ...
        % Realization tag                                
        tagR = [ 'f'       num2str( Out.Res( iR ).f, '%1.2f' ) ...
                 '_a'      num2str( Out.Res( iR ).a, '%1.2f' ) ...
                 '_dt'     num2str( Out.Res( iR ).dt ) ...
                 '_nS'     int2str( Out.Res( iR ).nS ) ...
                 '_nSSpin' int2str( Out.Res( iR ).nSSpin ) ...
                 '_r1'     num2str( Out.Res( iR ).r1, '%1.2f' ) ...
                 '_r2'     num2str( Out.Res( iR ).r2, '%1.2f' ) ...
                 'ifCent'  int2str( Out.Res( iR ).ifCent ) ];
