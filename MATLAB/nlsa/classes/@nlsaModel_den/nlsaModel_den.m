...
%
%   Contact: dimitrios.giannakis@dartmouth.edu
%
%   Modified 2023/09/06
    ...
    methods
        ...
        function obj = nlsaModel_den(varargin)
            ...
            if ~isempty(iDensity)
                ...
                if isa(varargin{iDensity}, 'nlsaKernelDensity_vb')
                    if any (getKNN(varargin{iDensity}) > nSETot)
                        error('The number of nearest neighbors cannot exceed the number of density samples')
                    end
                end
