...
% MAKEEMBEDDING Perform out-of-sample extension of the 
% lagged embedded data of an array of nlsaEmbeddedComponent_ose_n objects.
%
...
for iBG = Opt.batch
    ...
    tic
    phi = getEigenfunctions( diffOp, iB, iR )';
    tWall = toc;
    ...
    for iC = 1 : nC
        ...
        x = getProjectedData( src, iC, idxPhi ) * phi( idxPhi, : );
        tWall = toc;
        fprintf( logId, 'OSEX component %i/%i, realizationO %i/%i, batchO %i/%i %2.4f \n', iC, nC, iR, nR, iB, nB, tWall );

        tic 
        setData( obj( iC, iR ), x, iB )
        tWall = toc;
        fprintf( logId, 'WRITEX component %i/%i, realizationO %i/%i, batchO %i/%i $2.4f \n', iC, nC, iR, nR, iB, nB, tWall );
    end % component loop
