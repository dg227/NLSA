function computeData( obj, srcT, iCRec, iRRec, varargin )
% COMPUTEDATA Perform reconstruction in physical space from data in delay-
% coordinate spac
%
% Modified 2016/04/05
...
msgId = obj.getErrMsgId;
if ~isa( srcT, 'nlsaEmbeddedComponent' )
    msgStr = 'Second input argument must be an nlsaEmbeddedComponent object.'; 
    error( [ msgId 'invalidSrc' ], msgStr )
end
           
nD  = getDimension( obj );
nDE = getEmbeddingSpaceDimension( srcT( iCRec, 1 ) );
nE  = nDE / nD;
if  ~isposint( nE )
...
partitionE = getPartition( srcT( 1, : ) );
...
for iBRec = 1 : numel( Opt.batch )
    ...
    while deficit > 0
        ...
        if nSProvided <= 0
            ...
            xBE = getData( srcT( iCRec, iRRec ), iBE );
