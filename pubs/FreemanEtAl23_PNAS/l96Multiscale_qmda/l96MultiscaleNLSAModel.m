function [model, In, Out] = l96MultiscaleNLSAModel(In, Out)
% L96MULTISCALENLSAMODEL Build NLSA model for the Lorenz 96 multiscale 
% (2-level) system.
%
%  [model, In] = l96MultiscaleNLSAModel(In) builds an NLSA model based on 
%  the parameters specified in the structure In. 

%  [model, In, Out] = l96MultiscaleNLSAModel(In, Out) builds an NLSA model
...
%
%  l96MultiscaleNLSAModel uses the parameter values in In and Out to create 
%  arrays of nlsaComponent objects representing the in-sample and 
%  out-of-sample data, respectively. Then, it passses these arrays along with 
%  the parameter values to function /utils/nlsaModelFromPars. The latter, 
%  prepares appropriate arguments for the nlsaModel class constructors, and 
%  then calls the constructors to build the model.
%
%  l96MultiscaleNLSAModel is meant to be called by higher-level functions 
%  tailored to specific data analysis/forecasting experiments. 
%
...
if ifOse
    ...
    for iR = Out.nR : -1 : 1
        ...
        Out.Res(iR).nS = Out.Res(iR).nSProd; 
...
for iR = In.nR : -1 : 1
    ...
    % Realization tag                                
    tagR = ['nX'      int2str(In.nX) ...
            '_nY'     int2str(In.nY) ...
            '_F'      num2str(In.Res(iR).F, '%1.3g') ...
            '_hX'     num2str(In.Res(iR).hX, '%1.3g') ...
            '_hY'     num2str(In.Res(iR).hY, '%1.3g') ...
            '_eps'    num2str(In.Res(iR).epsilon, '%1.3g') ...
            '_dt'     num2str(In.dt, '%1.3g') ...
            '_x0'     num2str(In.Res(iR).x0(1), '%1.3g') ...
            '_y0'     num2str(In.Res(iR).y0(1), '%1.3g') ...
            '_nS'     int2str(In.Res(iR).nS) ...
            '_nSSpin' int2str(In.Res(iR).nSSpin) ...
            '_absTol' num2str(In.Res(iR).absTol, '%1.3g') ...
            '_relTol' num2str(In.Res(iR).relTol, '%1.3g') ...
            '_ifCent' int2str(In.Res(iR).ifCenter)];
    ...
    for iC = In.nC : -1 : 1
        ...
        % Component tag
        tagC = idx2str(In.Src(iC).idxX, 'idxX');

        % Component dimension
        nD = numel(In.Src(iC).idxX);

        % Input file
        if nD == In.nX && all(In.Src(iC).idxX == 1 : In.nX)
            fileName = 'dataX.mat';
        else
    ...
    for iC = In.nCT : -1 : 1
        ...
        % Component tag
        tagC = idx2str(In.Trg(iC).idxX, 'idxX');

        % Component dimension
        nD = numel(In.Trg(iC).idxX);

        % Input file
        if nD == In.nX && all(In.Trg(iC).idxX == 1 : In.nX)
            fileName = 'dataX.mat';
        else
            fileName = ['dataX_' tagC '.mat'];
        end
...
if ifOse
    for iR = Out.nR : -1 : 1
        ...
        % Realization tag                                
        tagR = ['nX'      int2str(Out.nX) ...
                '_nY'     int2str(Out.nY) ...
                '_F'      num2str(Out.Res(iR).F, '%1.3g') ...
                '_hX'     num2str(Out.Res(iR).hX, '%1.3g') ...
                '_hY'     num2str(Out.Res(iR).hY, '%1.3g') ...
                '_eps'    num2str(Out.Res(iR).epsilon, '%1.3g') ...
                '_dt'     num2str(Out.dt, '%1.3g') ...
                '_x0'     num2str(Out.Res(iR).x0(1), '%1.3g') ...
                '_y0'     num2str(Out.Res(iR).y0(1), '%1.3g') ...
                '_nS'     int2str(Out.Res(iR).nS) ...
                '_nSSpin' int2str(Out.Res(iR).nSSpin) ...
                '_absTol' num2str(Out.Res(iR).absTol, '%1.3g') ...
                '_relTol' num2str(Out.Res(iR).relTol, '%1.3g') ...
                '_ifCent' int2str(Out.Res(iR).ifCenter)];
        ...
        for iC = Out.nC : -1 : 1
            ...
            % Component tag
            tagC = idx2str(Out.Src(iC).idxX, 'idxX');

            % Component dimension
            nD = numel(Out.Src(iC).idxX);

            % Input file
            if nD == Out.nX && all(Out.Src(iC).idxX == 1 : Out.nX)
                fileName = 'dataX.mat';
            else
                fileName = ['dataX_' tagC '.mat'];
            end
        ...
        if ifOutTrg
            ...
            for iC = Out.nCT : -1 : 1
                ...
                % Component tag
                tagC = idx2str(Out.Trg(iC).idxX, 'idxX');

                % Component dimension
                nD = numel(Out.Trg(iC).idxX);

                % Input file
                if nD == Out.nX && all(Out.Trg(iC).idxX == 1 : Out.nX)
                    fileName = 'dataX.mat';
                else
                    fileName = ['dataX_' tagC '.mat'];
                end
