...
% query (out-of-sample) distance data distO. diffOp is the in-sample 
% diffusion operator. 
% 
% Modified 2020/01/30
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Read the right normalization vector 
if alpha ~= 0
    tic
...
for iBG = Opt.batch
    ...
    tWall = toc;
    fprintf( logId, 'EXPQ %i/%i %i/%i %2.4f \n', iR, nR, iB, nBR, tWall ); 

    tic
    % Perform left normalization by out-of-sample degree
    dO = getDegree( obj, iB, iR );
    pVal = bsxfun( @ldivide, dO, pVal );
    tWall = toc;
    fprintf( logId, 'COMPUTEP %i/%i %i/%i %2.4f \n', iR, nR, iB, nBR, tWall ); 
