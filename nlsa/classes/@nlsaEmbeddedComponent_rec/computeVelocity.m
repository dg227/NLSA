...
% COMPUTEVELOCITY Perform reconstruction of the 
% phase space velocity of an array of nlsaEmbeddedComponent_rec objects.
%
...
for iBG = Opt.batch
    ...
    % Read eigenfunctions for current batch, including the constant eigenfunction
    tic
    phi = getEigenfunctions( diffOp, true, iB, iR )';
    tWall = toc;
    fprintf( logId, 'READPHI realization %i/%i, batch %i/%i %2.4f \n', iR, nR, iB, nB, tWall );
    ...
    for iC = 1 : nC
        ...
        xiNorm2 = sum( xi .^ 2, 1 );
        tWall = toc;
        fprintf( logId, 'RECXI component %i/%i, realizationO %i/%i, batchO %i/%i %2.4f \n', iC, nC, iR, nR, iB, nB, tWall );

        tic `
        setVelocity( obj( iC, iR ), xiNorm2, xi, iB )
        tWall = toc;
        fprintf( logId, 'WRITEXI component %i/%i, realization %i/%i, batchO %i/%i $2.4f \n', iC, nC, iR, nR, iB, nB, tWall );
    end % component loop
