function [model, In, Out] = rosslerNLSAModel(In, Out)
% L63NLSAModel Build NLSA model for Rossler system
%
%  [model, In] = rosslerNLSAModel(In) builds an NLSA model based on the
%  parameters specified in the structure In.

%  [model, In, Out] = rosslerNLSAModel(In, Out) builds an NLSA model
...
%
%  rosslerNLSAModel uses the parameter values in In and Out to create arrays of
...
%
%  rosslerNLSAModel is meant to be called by higher-level functions tailored to
...
for iR = In.nR : -1 : 1
    ...
    % Realization tag
    tagR = ['a'       num2str(In.Res(iR).a, '%1.3g') ...
            '_b'      num2str(In.Res(iR).b, '%1.3g') ...
            '_c'      num2str(In.Res(iR).c, '%1.3g') ...
...
if ifOse
    for iR = Out.nR : -1 : 1
        ...
        % Realization tag
        tagR = ['a'    num2str(Out.Res(iR).a, '%1.3g') ...
                '_b'    num2str(Out.Res(iR).b, '%1.3g') ...
                '_c'  num2str(Out.Res(iR).c, '%1.3g') ...
