function [ model, In, Out ] = helixAmplitudeNLSAModel( In, Out )
% HELIXNLSAModel Build NLSA model for nonstationary helix system with amplitude
% trend.
%
%  [ model, In ] = helixAmplitudeNLSAModel( In ) builds an NLSA model based 
% on the parameters specified in the structure In. 

%  [ model, In, Out ] = helixAmplitudeNLSAModel( In, Out ) builds an NLSA model
...
%
%  helixAmplitudeNLSAModel uses the parameter values in In and Out to create 
...
%
%  helixAmplitudeNLSAModel is meant to be called by higher-level functions 
...
for iR = In.nR : -1 : 1
    ...
    lag    = round( pi / 2 / In.Res( iR ).dt );
    t      = 0 : In.Res( iR ).dt : In.Res( iR ).T; % timestamps  
    In.Res( iR ).nSProd = numel( t( 1 : end - 2 * lag ) );
...
if ifOse
    ...
    for iR = Out.nR : -1 : 1
        ...
        lag    = round( pi / 2 / Out.Res( iR ).dt );
        t      = 0 : Out.Res( iR ).dt : Out.Res( iR ).T; % timestamps  
        Out.Res( iR ).nSProd = numel( t( 1 : end - 2 * lag ) );
...
for iR = In.nR : -1 : 1
    ...
    % Realization tag                                
    tagR = [ 'a'       num2str( In.Res( iR ).a, '%1.2f' ) ...
             '_b'      num2str( In.Res( iR ).b, '%1.2f' ) ...
             '_T'      num2str( In.Res( iR ).T, '%1.2f' ) ...
             '_dt'     num2str( In.Res( iR ).dt ) ];
...
if ifOse
    for iR = Out.nR : -1 : 1
        ...
        % Realization tag                                
        tagR = [ 'a'       num2str( Out.Res( iR ).a, '%1.2f' ) ...
                 '_b'      num2str( Out.Res( iR ).b, '%1.2f' ) ...
                 '_T'      num2str( Out.Res( iR ).T, '%1.2f' ) ...
                 '_dt'     num2str( Out.Res( iR ).dt ) ];
        ...
        for iC = Out.nC : -1 : 1
            ...
            % Input file
            if nD == 2 && all( Out.Src( iC ).idxX == [ 1 2 ] )
                fileName = 'dataX.mat';
            else
                fileName = [ 'dataX_' tagC '.mat' ];
            end
        ...
        if ifOutTrg
            ...
            for iC = Out.nCT : -1 : 1
                ...
                % Input file
                if nD == 2 && all( Out.Trg( iC ).idxX == [ 1 2 ] )
                    fileName = 'dataX.mat';
                else
                    fileName = [ 'dataX_' tagC '.mat' ];
                end
