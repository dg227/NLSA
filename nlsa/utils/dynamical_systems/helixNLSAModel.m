function [ model, In, Out ] = helixNLSAModel( In, Out )
% HELIXNLSAModel Build NLSA model for nonstationary helix system.
%
%  [ model, In ] = helixNLSAModel( In ) builds an NLSA model based on the
%  parameters specified in the structure In. 

%  [ model, In, Out ] = helixNLSAModel( In, Out ) builds an NLSA model
...
%
%  helixNLSAModel uses the parameter values in In and Out to create arrays of 
...
%
%  helixNLSAModel is meant to be called by higher-level functions tailored to 
...
if ifOse
    for iR = Out.nR : -1 : 1
        ...
        % Realization tag                                
        tagR = [ '_a'      num2str( Out.Res( iR ).a, '%1.2f' ) ...
                 '_b'      num2str( Out.Res( iR ).b, '%1.2f' ) ...
                 '_c'      num2str( Out.Res( iR ).c, '%1.2f' ) ...
