...
experiment  = 'coffee';     % forecast model 
idxPhi      = 1 : 101;    % hypothesis space basis functions 
idxPhiVar   = 1 : 101;     % basis functions for conditional variance 
idxPhiProb  = 1 : 11;     % basis functions for conditional probability 
...
tNum0Plt    = '201301';   % initialization time to plot forecast
idxXPlt     = [ 4 ];      % target variable components to plot
tLimTruePlt = { '199707' '199907' }; % interval to plot true signal 
...
ifPlotPred        = false;   % prediction and skill metric plots
...
[ model, Pars, ParsO ] = ensoNLSAModel_den_ose( experiment );
...
if ifRead
    ...
    % verification signal
    tStr = strjoin( ParsO.Res( idxR ).tLim, '-' ); 
    ...
    tNumOut = model.outTime{ idxR }( idxEmb : end ); 
    toc
end
...
if ifPred
    ...
    fPred2 = reshape( fPred, [ nD nT nSO ] );
    toc
end
...
if ifPlotPred
    ...
    for iX = 1 : nTileX
        ...
        jdxTPltPred = idxXPlt( iX ) + ( 0 : nT - 1 ) * nX;
        ...
        plot( tVals, fRmse( idxXPlt( iX ), idxTPltPred ), 'b-' )
        hold on 
        plot( tVals, fRmsePred( idxXPlt( iX ), idxTPltPred ), 'c-' ) 
        ...
        plot( tVals, fPC( idxXPlt( iX ), idxTPltPred ), 'b-' )
        set( gca, 'xLimSpec', 'tight' )
...
if ifPlotRunning
    ...
    for iX = 1 : nTileX
        ...
        for iY = 1 : nTPlt
            ...
            jT = ( 1 : nSTO ) + idxTPltRunning( iY ) - 1;
            set( gcf, 'currentAxes', ax( iX, iY ) )
            [ hPredPlt, hErrPlt ] = boundedline( tNumPlt( jT ), fPred( jF, 1 : nSTO ),sqrt( fErrPred( jF, 1 : nSTO ) ), 'b-' );
            hold on
