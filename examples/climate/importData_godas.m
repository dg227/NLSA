function Data = importData_godas( DataSpecs )
% IMPORTDATA_GODAS Read monthly data from GODAS reanalysis netCDF files, and 
...
% 
% Longitude range is [ 0.5 359.5 ] 
% Latitude range is [ -74.5 64.5 ] 
%
% Modified 2020/05/19
...
endNum = datenum( Time.tEnd, Time.tFormat );
limNum = datenum( Time.tLim, Time.tFormat );
nTAvailable = months( startNum, endNum ) + 1;
...
if idxT1 < 1
    preDeficit = -idxT1 + 1; 
    idxT1 = 1;
    msgStr = [ 'Date range requested preceeds the available data. ' ...
           sprintf( 'Setting the first %i samples to zero.', preDeficit ) ]; 
    disp( msgStr )

else
    preDeficit = 0;
end
if idxTEnd > nTAvailable
    postDeficit = idxTEnd - nTAvailable;
    idxTEnd = nTAvailable;
    msgStr = [ 'Date range requested exceeds the available data. ' ...
...
% Append zeros before/after the data as needed
fld = [ zeros( nD, preDeficit ) fld zeros( nD, postDeficit ) ];
