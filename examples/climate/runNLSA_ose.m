function [ model, In ] = runNLSA_ose( experiment, iProc, nProc )
%
% This function creates an NLSA model with out-of-sample extension (OSE), 
% and executes the various NLSA steps for climate datasets
%
...
% climateNLSAModel_ose are changed, it is only necessary to repeat the steps 
...
%               the function climateNLSAModel_den
%
...
disp( experiment )
[ model, In ] = climateNLSAModel_ose( experiment ); 
...
disp( 'Phase space velocity, out-of-sample data' ); computeOutVelocity( model )


fprintf( 'OSE pairwise distances, %i/%i\n', iProc, nProc );  computeOsePairwiseDistances( model, iProc, nProc )
