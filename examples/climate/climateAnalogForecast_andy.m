...
experiment     = 'ip_sst_operational';     % forecast model 

idxPhi      = 1 : 201;    % hypothesis space basis functions 
idxPhiVar   = 1 : 201;    % basis functions for conditional variance 
...
%tNum0Plt    = '199701';   % initialization time to plot forecast
...
ifPlotRunning     = false; % running prediction plot
ifPlotProbRunning = false; % running condition probability plot
ifPlotOperational = true;  % "operational" prediction plots (without truth)
...
if ifPlotOperational
    ...
    idxTAndy = find( datenum( '202101', 'yyyymm' ) == tNumPred, 1, 'first' );
    tNumAndy = tNumPred( idxTAndy );
    ...
    for iX = 1 : nX
        ...
        hAndy = plot( [ tNumAndy tNumAndy ], yLim, 'r-', 'linewidth', 2 )
        if iX == 1
            hL = legend( [ hTruePlt hPredPlt hErrPlt hAndy ],  'true', 'forecast', 'estimated error', 'A. Majda''s 72nd birthday', 'location', 'northWest' );
        end
        if iX == 1
            ylabel( 'SST anomaly (K)' )
        end
...
%        set( gcf, 'children', aax( [ ind : end, 1 : ind - 1 ] ) )
    ...
    print -dpng -r300 figNino34Operational_andy.png
end
