...
switch experiment
    ...
    case 'quad_gyre_packed'
        ...
        % Source (covariate) data is sea ice concentration, sea ice velocity,
        % and ocean velocity
...
idxT1 = zeros( 1, In.nR );
for iR = In.nR : -1 : 1
    In.Res( iR ).nS = In.Res( iR ).tLim( 2 ) - In.Res( iR ).tLim( 1 ) + 1; % number of samples
...
for iR = Out.nR : -1 : 1
    Out.Res( iR ).nS = Out.Res( iR ).tLim( 2 ) - Out.Res( iR ).tLim( 1 ) + 1; % number of samples
