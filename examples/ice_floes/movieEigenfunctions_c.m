...
idxPhi = [ 2 : 9 ]; % eigenfunctions to plot
...
movieFile = 'moviePhi.avi';

Mov.figWidth   = 1000;    % in pixels
Mov.deltaX     = 40;
Mov.deltaX2    = 50;
Mov.deltaY     = 35;
Mov.deltaY2    = 40;
...
for iAx = 1 : Mov.nTileX
    for jAx = 1 : Mov.nTileY
        if sub2ind( [ Mov.nTileY Mov.nTileX ], jAx, iAx ) <= nPhiPlt + 2 
            Mov.ax( iAx, jAx ) = axes( ...
                    'units', 'pixels', ...
                    'position', ...
...
for iS = 1 : nS
    ...
    % kernel density
    pData( 1 : end - 1, 1 : end - 1 ) = squeeze( rho( iS, :, : ) ); 
    ...
    title( Mov.ax( 2, 1 ), 'kernel density' )
