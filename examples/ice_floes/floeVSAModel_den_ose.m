...
%        In.Src( 3 ).idxE      = 1 : 5; % delay embedding indices 
%        In.Src( 3 ).nXB       = 1;  % samples to leave out before main interval
%        In.Src( 3 ).nXA       = 1;  % samples to leave out after main interval
%        In.Src( 3 ).fdOrder   = 0;         % finite-difference order 
...
%        In.Src( 3 ).embFormat = 'overlap'; % storage format for delay embedding
    ...
    % CHANNEL CONFIGURATION -- CONCENTRATION AND OCEAN VELOCITY DATA
    case 'channel_cuvocn'
        ...
        In.Src( 1 ).field      = 'c';     % physical field
        In.Src( 1 ).nD         = 1;       % dimension
        In.Src( 2 ).field      = 'uvocn';    % physical field
        In.Src( 2 ).nD         = 2;       % dimension
        %In.Src( 3 ).field      = 'uvocn'; % physical field
        %In.Src( 3 ).nD         = 2;       % dimension
        %In.Trg( 1 ).field      = 'acc';   % physical field
...
outTrgComponent( Out.nCT, Out.nRG ) = nlsaComponent(); % target data
...
model = nlsaModel_den_ose( ... 
   ...
   'outTargetEmbeddingTemplate',      outTrgEmbComponent, ...
