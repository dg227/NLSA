function [ model, In, Out ] = pangeaNLSAModel( In, Out )
% CLIMATENLSAMODEL Low-level function to build NLSA models for PANGEA datasets
%
%  [ model, In ] = pangeaNLSAModel( In ) builds an NLSA model based on 
%  the model parameters specified in the structure In. 
%
%  [ model, In, Out ] = pangeaNLSAModel( In, Out ) builds an NLSA model
...
%
%  pangeaNLSAModel uses the parameter values in In and Out to create arrays of 
...
%
%  pangeaNLSAModel is meant to be called by higher-level functions tailored to 
...
for iR = In.nR : -1 : 1
    ...
    tStr = sprintf( '%i-%i', In.Res( iR ).tLim( 1 ), In.Res( iR ).tLim( 2 ) ); 
    tagR = tStr;
    ...
    for iC = In.nC : -1 : 1
        ...
        pathC = fullfile( inPath, In.Src( iC ).field, tStr );
    ...
    for iC = In.nCT : -1 : 1
        ...
        pathC = fullfile( inPath,  In.Trg( iC ).field, tStr );
...
if ifOse
    ...
    for iR = Out.nR : -1 : 1
        ...
        tStr = sprintf( '%i-%i', ...
                         Out.Res( iR ).tLim( 1 ), Out.Res( iR ).tLim( 2 ) );
        tagR = tStr;
        ...
        for iC = Out.nC : -1 : 1
            ...
            pathC = fullfile( outPath, Out.Src( iC ).field, tStr );
        ...
        if ifOutTrg
            ...
            for iC = Out.nCT : -1 : 1
                ...
                pathC = fullfile( outPath, Out.Trg( iC ).field, tStr );
