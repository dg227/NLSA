...
%
% This function creates a space-time NLSA model with kernel density estimation and 
% executes the various NLSA steps for data generated by the KS system
%
...
% ksNLSAModel_den are changed, it is only necessary to repeat the steps 
...
disp( experiment )
[ model, In ] = ksNLSAModel_den( experiment ); 
...
disp( 'Phase space velocity' ); computeVelocity( model, 1, 1, 1, 1, 'ifWriteXi', false )
...
computeDenPairwiseDistances( model, iProc, nProc, 1, 1, 'ifPartialSort', true )
...
computePairwiseDistances( model, iProc, nProc, 'ifPartialSort', true )
