...
% ccsmNLSAModel are changed, it is only necessary to repeat the steps 
...
% operator, run the following commands:
%
% 
% Diffusion operator (only available for gl_mb diffusion operators):
% [ epsilonOpt, Info ] = computeOptimalBandwidth( model );
%
% To recover the diffusion eigenfunctions, and the projected and reconstructed
...
disp( experiment )
[ model, In ] = ccsmNLSAModel( experiment ); 
