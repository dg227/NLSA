...
% executes the various NLSA steps for data generated by the dynamical system 
% on the 2-torus in torusData.m  
%
...
% torusNLSAModel_den are changed, it is only necessary to repeat the steps 
...
%               the function torusNLSAModel_den
%
...
disp( experiment )
[ model, In ] = torusNLSAModel_den( experiment ); 
...
if ifPlot
    ...
    rho = getDensity( model );                     % sampling density
    phi = getDiffusionEigenfunctions( model );     % in-sample eigenfunctions

    idxPhiPlot = 7; % eigenfunction to plot
    ...
    scatter3( x( 1, In.nXB + 1 : end - In.nXA ), ...
              ...
              x( 3, In.nXB + 1 : end - In.nXA ), 10, rho )
    view( 20, 80 )
    title( 'density' )
