...
%% EIGENFUNCTIONPARAMETERS
idxV      = 19 : 27;    % temporal patterns to plot
...
vT      = getDiffusionEigenfunctions( model );
tNum    = getSrcTime( model );
idxT1   = getOrigin( model.embComponent );
figDir  = fullfile( getDiffusionOperatorPath( model ), 'figures' );
figFile = [ 'figPhi',   sprintf( '_%i', idxV ), '.eps' ];
...
for iV = 1 : numel( idxV )
    ...
    hL = ylabel( [ '|FT({\phi}_{', int2str( idxV( iV ) ), '})|^2' ], 'fontSize', 7 );
    %hLP = get( hL, 'position' );
    %hLP( 1 ) = hLP( 1 ) * .75;
    %set( hL, 'position', hLP )
