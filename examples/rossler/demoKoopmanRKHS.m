% DEMO OF KOOPMAN SPECTRAL ANALYSIS FOR THE ROSSLER SYSTEM USING REPRODUCING
% KERNEL HILBERT SPACE (RKHS) COMPACTIFICATION
%
% This script computes approximate eigenvalues and eigenfunctions of the
% generator of the Koopman group on the L2 space associated with an invariant
% % measure on the Rossler attractor.
%
...
%
% '6.4k_dt0.04_nEL0':   6400 samples, sampling interval 0.01, 0 delays
% '6.4k_dt0.04_nEL800': 6400 samples, sampling interval 0.01, 800 delays
% '64k_dt0.04_nEL0':    64000 samples, sampling interval 0.01, no delays
% '64k_dt0.04_nEL800':  64000 samples, sampling interval 0.01, 800 delays
%
...
%experiment = '6.4k_dt0.04_nEL800';
%experiment = '64k_dt0.04_nEL0';
%experiment = '64k_dt0.04_nEL800';
...
if ifPlotCoherence
    ...
    title('(a) PC_1 on Rossler attracror')
    axis off
    view(0, 0)
    ...
    plot3(x(1, idxTPlt(1) : idxTPlt(2)), ...
           ...
           x(3, idxTPlt(1) : idxTPlt(2)), 'k-', ...
           'lineWidth', 2)
    title('(b) (PC_1, PC_2) angle on Rossler attractor')
    axis off
    view(90, 0)
    set(gca, 'cLim', [-pi pi ])
    colormap(gca, jet)
    ...
    %       x(3, idxTPlt(1) : iT), 'k-', ...
    %        'lineWidth', 2)
    title(sprintf('(e) \\phi_{%i} on Rossler attractor', idxZPlt(1) - 1))
    axis off
    view(0, 0)
    ...
    %       x(3, idxTPlt(1) : iT), 'm-', ...
    %        'lineWidth', 3)
    title(sprintf(['(f) (\\phi_{%i}, \\phi_{%i}) angle on Rossler ' ...
                      'attractor'], ...
...
if ifMovieCoherence
    ...
    for iFrame = 1 : nFrame
        ...
        title('(a) PC1 on Rossler attracror')
        axis off
        view(0, 0)
        set(gca, 'cLim', [-1 1] * aPC(1))
